...
///
/// Extracted from [ObsMcpServer] so the easing curves and interpolation
/// logic can be unit-tested without spinning up an MCP server or OBS
/// connection.
library;
...
///
/// Supported names: `linear`, `easeIn`, `easeOut`, `easeInOut`,
/// `easeOutBounce`. `null` is treated as `linear`.
///
...
double Function(double) resolveEasing(String? name) {
  switch (name) {
    ...
    default:
      throw ArgumentError(
        'Unknown easing "$name". Use linear|easeIn|easeOut|easeInOut|'
        'easeOutBounce.',
      );
  }
}
...
///
/// If both are null returns null. If one is null the other is returned
/// unchanged so partial transforms hold the steady value.
...
/// Integer companion to [lerpDouble]. Rounds to nearest int.
...
///
/// Numeric fields use [lerpDouble]/[lerpInt]. Non-numeric fields
/// (`alignment`, `boundsType`, `boundsAlignment`) snap to [to] on the
/// first frame so callers can change them in the same animation call.
...
/// of subscription [names]. Throws if both are supplied or if [names]
